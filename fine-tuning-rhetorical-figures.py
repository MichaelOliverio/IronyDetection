...
def prepare_dataset(split):
    ...
    for index, row in df.iterrows():
        dataset.append({
            'instruction': f'Given the ironic sentence (INPUT), identify and return the rhetorical figure it exemplifies in (OUTPUT).',
            'id': row['id'],
            'input': row['input'],
            'output': row['output'],
        })
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['instruction'])):
        text = f'''<s> [INST] {example['instruction'][i]} [/INST] [INPUT] {example['input'][i]} [/INPUT] [OUTPUT] {example['output'][i]} [/OUTPUT] </s>'''
...
def model_generation(pipe, test_dataset, output_path):
    ...
    for record in test_dataset:
        ...
        if not pipe:
            max_length = int(len(f"<s> [INST] {record['instruction']} [/INST] [INPUT] {record['input']} [/INPUT] [OUTPUT] {record['output']} [/OUTPUT] </s>") * 1.5)
        ...
        df = pd.DataFrame(list(zip(ids, inputs, predictions, actuals, generations)), columns=['ids', 'input', 'prediction', 'actual', 'generation'])
...
print(f'eval len: {len(eval_dataset)}')
