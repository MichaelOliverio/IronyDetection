...
data_path_prediction = './models/multipico_generation'
...
#        "You are given in input (INPUT) a post extracted from social media conversations. Provide in output (OUTPUT) an ironic reply in Italian."
    instruction = (
        "Ti viene fornito in input (INPUT) un post estratto da conversazioni sui social media. Fornisci in output (OUTPUT) una risposta ironica in italiano."
        )
...
def model_generation(pipe, test_dataset, output_path):
    ...
    with torch.no_grad():
        for i, record in enumerate(test_dataset, 1):
            ...
            generation = clear_output(result[0]['generated_text'])
            print(f'Result: {result} | Output: {result[0]["generated_text"]} | Generation: {generation}')
            records.append((record['reply_id'], record['post_id'], record['input'], generation, result[0]['generated_text']))

            if i % 5 == 0:
                gc.collect()
                torch.cuda.empty_cache()

        df = pd.DataFrame(records, columns=['reply_id','post_id', 'input', 'generation', 'raw_generation'])
